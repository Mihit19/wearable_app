...
class _MonitoringDashboardPageState extends State<MonitoringDashboardPage> {
  ...
  void initState() {
    ...
    databaseRef.child('healthMonitoring').onValue.listen((event) {
      final data = event.snapshot.value as Map?;
      if (data != null) {
        setState(() {
          bodyTemperature = data['bodyTemperature']?.toDouble() ?? bodyTemperature;
          ambientTemperature = data['ambientTemperature']?.toDouble() ?? ambientTemperature;
          heartRate = data['heartRate']?.toInt() ?? heartRate;
          isOn = data['isOn'] ?? isOn;
        });
      }
    });
  ...
  void togglePower() {
    ...
    databaseRef.child('healthMonitoring/isOn').set(isOn);
  }
